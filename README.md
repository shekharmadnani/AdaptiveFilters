...
| **Learned DCT-domain Wiener filter** | [learned/wiener.py](adaptive_filters/learned/wiener.py) + [train_wiener.py](train_wiener.py) → `models/wiener.pt` | CNN predicts per-coefficient gains in [0,1] (attenuate-only shrinkage; K-map is the binary special case); paired training by default | primary residual generator; K_g = sum of gains is the continuous effective-K |
